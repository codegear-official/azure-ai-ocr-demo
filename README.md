...
インストール後に「コンピューター証明書の管理」管理コンソールアプリを実行し、「信頼されたルート証明機関」の下の「証明書」の一覧の中に発行先と発行者が共に「hron」になっている証明書が見つかると思います。このテスト用オレオレ証明書は有効期限が1年に設定されているので、証明が有効なのは証明書生成後、最大1年間ということになります。
