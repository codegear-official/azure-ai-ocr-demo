...
もしVisual Studio 2019 Comunity Editionを利用したい場合は、現在通常の Visual Studio Community のページからはダウンロードできなくなっています。
...
このドキュメントではメニューなどのUIを英語で表示しています。もし英語表示を行う場合には「言語パック」で「英語」を追加してインストールした後、Visual Studio IDEの Toolsメニューの Options で国別設定を表示し、Englishを選んでIDEを再起動すると英語UIになります。
...
何らかの方法で「Windows カメラ」アプリをインストールすれば「Take Picture」は動作します。あるいは (LTSCではなく) SACであればMicrosoft StoreおよびプリインストールのStoreアプリも存在するので、通常のWindowsと同様に動作します。
